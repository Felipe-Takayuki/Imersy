...
func (udb *UserDB) GetProjects() ([]*model.Project, error) {
	query:= `
...
func (udb *UserDB) SendEvaluationProject(evaluatorID, projectID, qualityGrade, creativityGrade int64)  error {
	_, err := udb.db.Exec("DELETE FROM SUBMITTED_PROJECT where project_id = ?", projectID)
	if err != nil {
		return err 
	}
	...
	_, err = udb.db.Exec("UPDATE PROJECT SET status = ? where id = ?", "avaliado",projectID)
	if err != nil {
		return err 
	}
	return nil 
...
func (udb *UserDB) GetTopRankProjectsByCategorie(categorie string) ([]*model.Project, error) {
	...
	FROM PROJECT p
	JOIN EVALUTED_PROJECT ep ON p.id = ep.project_id
